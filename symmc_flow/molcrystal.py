...
# ===================== relative-orientation gauge ============================
def _species_groups(item):
    """Group the real molecule slots of an item by shared conformer (i.e. same chemical
    species). Copies of one species share EXACTLY one `local` (the registry reference), so
    equal `atom_mask` + allclose `local` identifies a species group. Returns a list of slot
    lists, each in ascending slot order (so group[0] is the first-detected copy)."""
    mol_mask, local, atom_mask = item["mol_mask"], item["local"], item["atom_mask"]
    slots = [m for m in range(mol_mask.shape[0]) if bool(mol_mask[m])]
    groups, used = [], set()
    for m in slots:
        if m in used:
            continue
        g, used = [m], used | {m}
        for n in slots:
            if n not in used and torch.equal(atom_mask[m], atom_mask[n]) \
                    and torch.allclose(local[m], local[n], atol=1e-5):
                g.append(n)
                used.add(n)
        groups.append(g)
    return groups
...
def species_multiplicity(item):
    """Largest number of symmetry copies of any one species in this crystal (>=2 means a
    non-trivial within-crystal relative-orientation target exists)."""
    return max((len(g) for g in _species_groups(item)), default=0)


def relative_gauge_item(item):
    """Re-gauge orientations to the WITHIN-CRYSTAL RELATIVE convention.

    The loader's default `orient` is the absolute pose R_m of copy m vs the molecule-intrinsic
    body frame. For a copy generated by space-group op g_m from an asymmetric-unit molecule of
    pose R_asym, R_m = rot(g_m) @ R_asym: the rot(g_m) part is determined by the space group
    (learnable) but R_asym is a per-crystal FREE orientation (gauge-arbitrary, unlearnable) and
    it dominates the SO(3) target -> the predict-zero floor. Picking the first copy of each
    species as the reference (orient := I) and expressing the rest RELATIVE to it cancels
    R_asym, leaving only the symmetry-induced relative rotations:

        R'_m = R_m @ R0^{-1},   local'_m = R0 @ local_m   (so R'_m @ local'_m = R_m @ local_m)

    Reconstruction is unchanged (gauge-only transform); the EGNN conformer embedding is
    rotation-invariant so the molecule identity is unchanged. Adds `is_ref` (M,) bool: True for
    each species' reference copy (target I, trivial) so an eval can isolate the genuinely
    symmetry-determined non-reference targets. Returns a new item (inputs untouched)."""
    out = {k: (v.clone() if torch.is_tensor(v) else v) for k, v in item.items()}
    orient, local = out["orient"], out["local"]
    is_ref = torch.zeros(orient.shape[0], dtype=torch.bool)
    for g in _species_groups(out):
        R0 = orient[g[0]].clone()
        is_ref[g[0]] = True
        for m in g:
            orient[m] = orient[m] @ R0.transpose(-1, -2)   # R'_m = R_m R0^{-1}
            local[m] = local[m] @ R0.transpose(-1, -2)     # local'_m = R0 @ local_m
    out["is_ref"] = is_ref
    return out


def assign_cosets(items, angle_tol_deg: float = 20.0):
    """Attach a per-molecule space-group COSET id to each relative-gauge item (2c diagnostic).

    For a copy rigidly generated by a crystallographic op, its relative rotation R'_m equals
    the op's (Cartesian) rotation part, so within one space group the non-reference relative
    rotations fall into a small discrete set of cosets. This greedily clusters them per space
    group (within `angle_tol_deg` geodesic) into a shared codebook and writes `coset` (M,) long:
    0 for reference/padding molecules, a global id >=1 for each non-reference coset. Conditioning
    the field on this discrete id (a standard CSP-style label, NOT the continuous rotation) tests
    whether the SO(3) head can then reproduce rot(g_m) near-exactly. Operates in place and also
    returns (items, n_cosets). Items must carry `is_ref` (call `relative_gauge_item` first)."""
    tol = angle_tol_deg * 3.141592653589793 / 180.0
    book: list[tuple[int, torch.Tensor]] = []   # (sg, representative rotation) -> id = index+1
    for it in items:
        sg = int(it["sg"])
        coset = torch.zeros(it["orient"].shape[0], dtype=torch.long)
        for m in range(it["orient"].shape[0]):
            if not bool(it["mol_mask"][m]) or bool(it["is_ref"][m]):
                continue
            R = it["orient"][m]
            gid = None
            for j, (sg_j, R_j) in enumerate(book):
                if sg_j == sg and float(M.so3_angle(R, R_j)) < tol:
                    gid = j + 1
                    break
            if gid is None:
                book.append((sg, R.clone()))
                gid = len(book)
            coset[m] = gid
        it["coset"] = coset
