% =====================================================================
% Deployable Symmetry-Coset Conditioning for Orientation Flows in
...
\title{\bfseries Deployable Symmetry-Coset Conditioning for Orientation
Flows in Molecular-Crystal Structure Prediction}
...
gap. The method converts the inference-limited part of the orientation field into
a near-deterministic, template-available lookup, leaving only the free pose to be
sampled.
\end{abstract}
...
while the orientation error is only $13.4^\circ$. Our contribution is therefore
scoped to the orientation mechanism, which this work advances; joint
lattice--centroid generation at scale remains the limiting factor, consistent
with the modest ($\sim\!1{,}000$-crystal) training budget.
...
gauge-free asymmetric-unit pose. The main open problems are template-free coset
prediction (here $39.5\%$, not yet sufficient) and end-to-end lattice--centroid
generation, which bounds exact reconstruction independently of orientation. The
...
$41^\circ$ template-supplied). \textbf{End-to-end} (all manifolds sampled,
best-of-$20$): $0\%$ match; median errors lattice-param $0.44$, centroid $0.35$,
orientation $13.4^\circ$.
