...
stable. The signal therefore rides on the space group, which is conditioned
...
training, a wider network with more attention layers trained for more than twice
as many steps, raises the non-reference signal from $27\%$ to $33\%$. This is
real but diminishing headroom that plateaus well above zero, so the relative
target is not merely under-fit; scaling alone does not close it.
...
conditioning raises the non-reference signal to $51\%$, with reference copies
...
(Fig.~\ref{fig:results}b); over three seeds the coset-conditioned non-reference
drop is $47.9\pm3.2\%$. Most of the ceiling is therefore the difficulty of
...
coset index is derived in part from the target it helps predict, so the $51\%$ is
...
$\mathrm{SO}(3)$ sampled; best of eight draws; $n=131$ held-out crystals). The
trained flow reconstructs $16.8\%$ of packings exactly, against $0\%$ for the
predict-floor field and $1.5\%$ for the naive $R=I$ baseline. (b) Held-out
...
$n=131$). The best-of-$20$ per-component errors locate the bottleneck: a median
lattice-parameter error of $0.46$, a fractional centroid error of $0.35$, and an
orientation error of $60^\circ$. Every head contributes; in particular the
...
component. The orientation-isolated $16.8\%$ is therefore an upper bound that is
attainable only when the packing is supplied, and end-to-end molecular-crystal
generation at this corpus scale and compute budget remains open. This is
consistent with the scope of the paper: the contribution is the characterization
of the orientation field, not a finished generator.
...
rejected. These choices keep the rigid-body benchmark honest but narrow its
coverage.
...
than sampling noise. The baseline relative run uses the default network for
$800$ steps; the capacity run widens the model and adds attention and encoder
depth and trains for $2{,}000$ steps.
